...
class SMTPCheck:
    ...
    def __init__(self, host, port, from_addr, to_addr, exfil_payload, encryption,
                 username=None, password=None, subject=None, body=None, timeout=DEFAULT_TIMEOUT):
        """
        Arguments:
            host - SMTP host
            port - SMTP port
        ...
        if encryption not in self.ENCRYPTION_OPTIONS:
            raise ValueError('SMTPCheck.encryption argument must '
    ...
    def build_msg(self, from_addr, to_addr, body=None, subject=None, exfil_payload=None):
        msg = MIMEMultipart()
        msg['From'] = from_addr
        msg['To'] = to_addr
        ...
        if exfil_payload:
            if exfil_payload.exfil_mode == 'attachment':
                multipart = MIMEApplication(exfil_payload.read(), Name=exfil_payload.filename)
                multipart['Content-Disposition'] = f'attachment; filename="{exfil_payload.filename}"'
                msg.attach(multipart)
            else:
                body_raw += '\r\r' + exfil_payload.read().decode('utf8', errors='replace')
    ...
    def exfil(self, payload):
        ...
        try:
            with smtp_client(self.host, self.port, timeout=self.timeout) as smtp:
                ...
                msg = self.build_msg(self.from_addr, self.to_addr, self.body, self.subject, payload)
    ...
    def check(self):
        success = self.exfil(self.exfil_payload)
        if success is True:
